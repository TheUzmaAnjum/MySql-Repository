...
-- NEW says only from the new rows that were inserted. There is also OLD which is rows that were deleted or updated, but for now we want NEW
...
-- now it was updated in the salary table 
...
-- Events are task or block of code that gets executed according to a schedule. In other words,Importing data on a schedule. 
